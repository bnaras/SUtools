...
gen_registration <- function(pkg_name, fun_list, outputFile = paste0(pkg_name, "_init.c")) {
    ...
    prolog <- gsub(pkg_name_pattern_lc, pkg_name_lc, prolog)
...
make_c_args <-    function(...){
    ...
    start_char <- substring(args, 1, 1)
    int_or_real <- match(start_char, c("i","j","k","l","m","n") , 0) > 0
    type <- ifelse(int_or_real, "int", "double")
    paste0(type, " *", args)
}

#' Generate function name and C arg types from subroutine definition
#'
...
make_fun_list <- function(fun_strs) {
    lapply(fun_strs,
           function(fun_str) {
               fun <- stringr::str_match(fun_str, "subroutine\\s+(\\w+)\\(")[, 2]
