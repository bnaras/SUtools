...
While this package is directed specifically towards Mortran, the
registration function `gen_registration` can be used by others for
ordinary Fortran.
...
along a path of values for $\lambda$.  We'll use a temporary
directory, so as not to clutter the current working directory.
...
This will generate two files `pcLasso.f` and `pcLasso_init.c` in the
output directory that can be copied over the `src` subdirectory of the
package `pcLasso`.
...
- If one chooses `register = TRUE`, registration for all the
